...
import com.github.kadehar.simpleresttodolist.todo.Todo;
...
public final class TodoRestController {
    ...
    public ResponseEntity<List<Todo>> getTodoList() {
        return ResponseEntity.ok(
                this.jdbcOperations.query("select * from todo", ((resultSet, i) ->
        ...
        try {
            return ResponseEntity.ok(
                    Objects.requireNonNull(
                            this.jdbcOperations.queryForObject("select * from todo where id = ? limit 1",
                                    (resultSet, i) ->
                                            new Todo(UUID.fromString(resultSet.getString("id")),
                                                    resultSet.getTimestamp("date_created").toLocalDateTime(),
                                                    resultSet.getBoolean("done"),
    ...
    public ResponseEntity<Todo> createTodo(@RequestBody TodoPayload payload) {
        Todo todo = new Todo(payload.isDone(), payload.getTask());
    ...
    public ResponseEntity<Void> modifyTodo(@PathVariable UUID todoId, @RequestBody TodoPayload payload) {
        if (this.jdbcOperations.update("update todo set done = ?, task = ? where id = ?",
                payload.isDone(), payload.getTask(), todoId.toString()) == 1) {
